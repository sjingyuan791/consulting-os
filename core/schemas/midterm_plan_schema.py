...
class RootCauseAnalysis(BaseModel):
    """根本原因分析セクション"""
    primary_symptom: str = Field(default="構造的課題（詳細未定義）", description="主要症状")
...
class CorporateStrategySection(BaseModel):
    """全社戦略セクション"""
    strategic_intent: str = Field(default="基本戦略方針（詳細未定義）", description="戦略意図")
...
class GenerationMetadata(BaseModel):
    """
    AI生成時の監査メタデータ
    - model_name: 使用したモデル名 (e.g., "gpt-4o-2024-05-13")
    - prompt_snapshot: 生成に使用したプロンプトのスナップショット
    - generated_at: 生成日時
    - usage: トークン使用量 (prompt_tokens, completion_tokens, total_tokens)
    - finish_reason: 生成終了理由
    - system_fingerprint: OpenAIシステムフィンガープリント
    """
    model_name: str = Field(..., description="使用モデル名")
    prompt_snapshot: str = Field(..., description="プロンプトスナップショット")
    generated_at: str = Field(..., description="生成日時(ISO8601)")
    usage: Dict[str, int] = Field(default_factory=dict, description="トークン使用量")
    finish_reason: Optional[str] = Field(default=None, description="終了理由")
    system_fingerprint: Optional[str] = Field(default=None, description="システムフィンガープリント")


class FeedbackItem(BaseModel):
    """フィードバックアイテム"""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="フィードバックID")
    timestamp: str = Field(default_factory=lambda: datetime.now().isoformat(), description="投稿日時")
    user_id: Optional[str] = Field(default=None, description="ユーザーID")
    content: str = Field(..., description="フィードバック内容")
    resolved: bool = Field(default=False, description="解決済みフラグ")
    feedback_type: Optional[str] = Field(
        default=None,
        description="修正カテゴリ: direction_change/fact_correction/tone_adjustment/info_addition/other"
    )
    provided_by: Optional[str] = Field(default=None, description="提供者 (consultant/system)")
    resulted_in_regeneration: bool = Field(default=False, description="このFBが再生成を引き起こしたか")
...
class MidtermPlanSection(BaseModel):
    ...
    generation_metadata: Optional[GenerationMetadata] = Field(
        default=None, description="生成監査メタデータ"
    )
    feedback_history: List[FeedbackItem] = Field(
        default=[], description="フィードバック履歴"
    )
    ai_draft_snapshot: Optional[str] = Field(
        default=None, description="AI生成直後の原文スナップショット（Diff表示用）"
    )
...
class MidtermPlanDocument(BaseModel):
    ...
    # Financial Diagnosis Persistence
    financial_diagnosis: Optional[List[FinancialMetrics]] = Field(
        default=None, description="財務データ診断結果（時系列）"
    )
